...
function AnimalInfoDialog.show(farmId, uniqueId, children, animalType, identifiers)
	...
	if AnimalInfoDialog.INSTANCE == nil then
		AnimalInfoDialog.register()
	end
	...
	if success then
		g_gui:showDialog("AnimalInfoDialog")
	end
...
function AnimalInfoDialog:setChildren(children)
	...
	for _, placeable in ipairs(placeables) do
		...
		if placeable.spec_husbandryAnimals ~= nil then
			if placeable.spec_husbandryAnimals.animalTypeIndex ~= self.animalType then
				continue
			end
	...
	if foundChildren ~= nil and #foundChildren > 0 then
		self.childrenSelector:setState(1)
	end
...
function AnimalInfoDialog:onClickItems(index, _, _)
	...
	if self.children == nil or #self.children == 0 or self.children[index] == nil then
		return
	end
...
function AnimalInfoDialog:updateContent(farmId, uniqueId, useChildren)
	...
	if useChildren then
		...
		for _, placeable in ipairs(placeables) do
			...
			for _, animal in ipairs(animals) do
				...
				if animal.farmId ~= farmId or animal.uniqueId ~= uniqueId then
					continue
				end

				--parent = animal
				table.insert(foundAnimals, animal)
				if self.children == nil or #self.children <= 1 then
					break
				end
			end
	...
	if parent == nil then
		InfoDialog.INSTANCE:setText(g_i18n:getText("el_ui_cantFindAnimal"))
		g_gui:showDialog("InfoDialog")
		return false
	end
	...
	local text = {
		title = g_i18n:getText("el_ui_uniqueId"),
		text = uniqueId
	}
	...
	text = {
		title = g_i18n:getText("el_ui_farmId"),
		text = farmId
	}
	...
	text = {
		title = g_i18n:getText("el_ui_gender"),
		text = g_i18n:getText("el_ui_" .. parent.gender)
	}
	...
	text = {
		title = g_i18n:getText("el_ui_value"),
	...
	text = {
		title = g_i18n:getText("el_ui_valuePerKilo"),
	...
	if parent.gender == "female" then
		...
		local canReproduce = g_i18n:getText("el_ui_yes")
		...
		if parent.age >= subType.reproductionMinAgeMonth then
			text = {
				...
				text = parent.isPregnant and g_i18n:getText("el_ui_yes") or g_i18n:getText("el_ui_no")
			}
		...
		if parent.isPregnant then
			text = {
				title = g_i18n:getText("el_ui_impregnatedBy"),
				text = (parent.impregnatedBy ~= nil and parent.impregnatedBy.uniqueId ~= "-1") and parent.impregnatedBy.uniqueId or g_i18n:getText("el_ui_unknown")
			}
