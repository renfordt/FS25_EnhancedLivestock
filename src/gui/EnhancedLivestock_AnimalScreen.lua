...
function EnhancedLivestock_AnimalScreen:onGuiSetupFinished()
	...
	local geneticTexts = {
		...
		getText("el_ui_genetics_bad"),
		getText("el_ui_genetics_average"),
		getText("el_ui_genetics_high"),
		getText("el_ui_genetics_veryHigh"),
		getText("el_ui_genetics_extremelyHigh"),
		getText("el_ui_genetics_highest")
	}

	local fertilityTexts = table.clone(geneticTexts, 1)
	table.insert(fertilityTexts, 1, getText("el_ui_genetics_infertile"))
	...
	self.herdsmanOptions = {
		...
		["budget|type"] = { ["target"] = "budget|type", ["type"] = "binary", ["values"] = { "fixed", "percentage" }, ["texts"] = { getText("el_ui_fixed"), getText("el_ui_percentage") } },
		...
		["mark"] = { ["target"] = "mark", ["type"] = "binary", ["values"] = { false, true }, ["texts"] = { getText("el_ui_dontMark"), getText("el_ui_mark") } },
		["diseases"] = { ["target"] = "diseases", ["type"] = "binary", ["values"] = { false, true }, ["texts"] = { getText("el_ui_noDiseases"), getText("el_ui_any") } },
		["diseasesSecondary"] = { ["target"] = "diseases", ["type"] = "binary", ["values"] = { false, true }, ["texts"] = { getText("el_ui_any"), getText("el_ui_onlyDiseases") } },
		["gender"] = { ["target"] = "gender", ["type"] = "tripleOption", ["values"] = { "female", "any", "male" }, ["texts"] = { getText("el_ui_female"), getText("el_ui_any"), getText("el_ui_male") } },
		...
		["convention"] = { ["target"] = "convention", ["type"] = "binary", ["values"] = { "random", "alphabetical" }, ["texts"] = { getText("el_button_random"), getText("el_ui_alphabetical") } }
	}
...
function AnimalScreen:updateLog()
	...
	self.messageListMessageNumber:setText(string.format(g_i18n:getText("el_ui_messageNumber"), (#self.messages[self.currentMessagePage] == 0 and 0 or 1) + 250 * (self.currentMessagePage - 1), (self.currentMessagePage - 1) * 250 + #self.messages[self.currentMessagePage], totalNumMessages))
...
function AnimalScreen:onSemenBought(errorCode)
	...
	elseif errorCode == AnimalBuyEvent.BUY_ERROR_NO_PERMISSION then
		dialogType = DialogElement.TYPE_WARNING
		text = "el_ui_semenPurchaseNoPermission"
	...
	if farmDewars ~= nil then
		...
		if dewars ~= nil then
			...
			for _, dewar in pairs(dewars) do
				...
				table.insert(dewarTexts, string.format("%s %s %s (%s %s)", EnhancedLivestock.AREA_CODES[dewar.animal.country].code, dewar.animal.farmId, dewar.animal.uniqueId, dewar.straws, g_i18n:getText("el_ui_straw" .. (dewar.straws == 1 and "Single" or "Multiple"))))
...
AnimalScreen.changeName = EnhancedLivestock_AnimalScreen.changeName
...
function EnhancedLivestock_AnimalScreen:onClickAnimalInfo(button)
	...
	--end
		...
		if item ~= nil then
			...
			if self.isInfoMode then
				...
				self.motherInfoButton:setText(g_i18n:getText("el_ui_mother") .. " (" .. ((animal.motherId == nil or animal.motherId == "-1") and g_i18n:getText("el_ui_unknown") or animal.motherId) .. ")")

				self.fatherInfoButton:setDisabled(animal.fatherId == nil or animal.fatherId == "-1")
				self.fatherInfoButton:setText(g_i18n:getText("el_ui_father") .. " (" .. ((animal.fatherId == nil or animal.fatherId == "-1") and g_i18n:getText("el_ui_unknown") or animal.fatherId) .. ")")
				...
				for fillType, amount in pairs(animal.input) do
					...
					title:setText(g_i18n:getText("el_ui_input_" .. fillType))
					value:setText(string.format(g_i18n:getText("el_ui_amountPerDay"), (amount * 24) / daysPerMonth))
				...
				for fillType, amount in pairs(animal.output) do
					...
					title:setText(g_i18n:getText("el_ui_output_" .. outputText))
					value:setText(string.format(g_i18n:getText("el_ui_amountPerDay"), (amount * 24) / daysPerMonth))
...
function EnhancedLivestock_AnimalScreen:populateCellForItemInSection(_, list, _, index, cell)
	...
	if list == self.husbandryList then
		...
		cell:getAttribute("importance"):setImageSlice(nil, "enhanced_livestock.importance_" .. baseMessage.importance)
	...
	if list == self.sourceList then
		...
		elseif isMarked then
			...
			descriptor:setText(g_i18n:getText("el_mark_" .. markText))
		...
		if (self.isInfoMode and not self.isBuyMode) or self.isTrailer then
			...
			if check ~= nil then
				...
				local selectAllText = g_i18n:getText("el_ui_selectAll")
				local selectNoneText = g_i18n:getText("el_ui_selectNone")
