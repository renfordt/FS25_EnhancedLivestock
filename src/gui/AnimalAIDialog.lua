...
function AnimalAIDialog:populateCellForItemInSection(list, section, index, cell)
	...
	cell:getAttribute("identifier"):setText(string.format("%s %s %s", EnhancedLivestock.AREA_CODES[animal.country].code, animal.farmId, animal.uniqueId))
...
function AnimalAIDialog:onClickSortButton(button)
	...
	table.sort(self.dewars, function(a, b)
		...
		if target == "identifier" then
			...
			aTarget = string.format("%s %s %s", EnhancedLivestock.AREA_CODES[a.animal.country].code, a.animal.farmId, a.animal.uniqueId)
			bTarget = string.format("%s %s %s", EnhancedLivestock.AREA_CODES[b.animal.country].code, b.animal.farmId, b.animal.uniqueId)
