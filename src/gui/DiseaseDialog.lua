...
function DiseaseDialog:onClickListItem(index)
	...
	self.yesButton:setText(g_i18n:getText("el_ui_" .. (disease.beingTreated and "stop" or (disease.treatmentDuration > 0 and "resume" or "start")) .. "Treatment"))
...
function DiseaseDialog:populateCellForItemInSection(list, section, index, cell)
	...
	cell:getAttribute("duration"):setText(treatment == nil and "N/A" or EnhancedLivestock.formatAge(treatment.duration - disease.treatmentDuration))
	cell:getAttribute("fee"):setText(treatment == nil and "N/A" or string.format(g_i18n:getText("el_ui_feePerMonth"), g_i18n:formatMoney(treatment.cost, 2, true, true)))
