...
MPLoadingScreen.update = Utils.overwrittenFunction(MPLoadingScreen.update, function(self, superFunc, dT)
    ...
    if not self.verifiedDependencies then
        ...
        if isIncompatible then
            ...
            local text = g_i18n:getText("el_ui_dependencies_missing") .. "\n"

            for _, dependency in pairs(dependencies) do
                if not dependency.installed then
                    text = text .. "\n" .. string.format(g_i18n:getText("el_ui_dependency_missing_notInstalled"), dependency.name)
                elseif dependency.incompatible then
                    text = text .. "\n" .. string.format(g_i18n:getText("el_ui_dependency_missing_installed"), dependency.name, dependency.version, dependency.minVersion)
                end
            end
