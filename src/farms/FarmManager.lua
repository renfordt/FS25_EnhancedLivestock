...
function EL_FarmManager:loadFromXMLFile(superFunc, path)
	...
	Logging.info("[EnhancedLivestock] FarmManager: loadFromXMLFile called")
	...
	if g_currentMission:getIsServer() then
		Logging.info("[EnhancedLivestock] FarmManager: Running on server, checking migration state...")

		-- Create migration manager instance if not already created by RmItemSystemMigration
		if g_ElMigrationManager == nil then
			Logging.info("[EnhancedLivestock] FarmManager: Creating ElMigrationManager instance")
		...
		-- Conflict detected - will show dialog in onStartMission
			Logging.warning("[EnhancedLivestock] FarmManager: Conflict detected!")
		...
		-- This is a fallback in case ItemSystem hook didn't run
			Logging.warning("[EnhancedLivestock] FarmManager: Migration needed (fallback path)!")
			g_elPendingMigration = true
		else
			Logging.info("[EnhancedLivestock] FarmManager: No conflict detected, migration state = " .. tostring(g_elPendingMigration))
		end

		Logging.info("[EnhancedLivestock] FarmManager: g_elMigrationConflict = " .. tostring(g_elMigrationConflict))
		Logging.info("[EnhancedLivestock] FarmManager: g_elPendingMigration = " .. tostring(g_elPendingMigration))
	else
		Logging.info("[EnhancedLivestock] FarmManager: Not running on server, skipping migration check")
	end
