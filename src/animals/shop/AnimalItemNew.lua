...
function AnimalItemNew.new(animal)
    ...
    if breederQuality >= 1.65 then
        breederQualityString = g_i18n:getText("el_ui_genetics_extremelyGood")
    elseif breederQuality >= 1.4 then
        breederQualityString = g_i18n:getText("el_ui_genetics_veryGood")
    elseif breederQuality >= 1.1 then
        breederQualityString = g_i18n:getText("el_ui_genetics_good")
    elseif breederQuality >= 0.9 then
        breederQualityString = g_i18n:getText("el_ui_genetics_average")
    elseif breederQuality >= 0.7 then
        breederQualityString = g_i18n:getText("el_ui_genetics_bad")
    elseif breederQuality >= 0.35 then
        breederQualityString = g_i18n:getText("el_ui_genetics_veryBad")
    else
        breederQualityString = g_i18n:getText("el_ui_genetics_extremelyBad")
    end

    self.title = g_fillTypeManager:getFillTypeTitleByIndex(subType.fillTypeIndex)
    self.infos = {
        {
            ["title"] = g_i18n:getText("el_ui_breederQuality"),
            ["value"] = breederQualityString
        },
        {
            ["title"] = g_i18n:getText("el_ui_animalOrigin"),
            ["value"] = EnhancedLivestock.AREA_CODES[countryIndex].country
        },
        {
    ...
    for key, value in pairs(genetics) do
        ...
        if value >= 1.65 then
            qualityText = g_i18n:getText("el_ui_genetics_extremelyHigh")
        elseif value >= 1.4 then
            qualityText = g_i18n:getText("el_ui_genetics_veryHigh")
        elseif value >= 1.1 then
            qualityText = g_i18n:getText("el_ui_genetics_high")
        elseif value >= 0.9 then
            qualityText = g_i18n:getText("el_ui_genetics_average")
        elseif value >= 0.7 then
            qualityText = g_i18n:getText("el_ui_genetics_low")
        elseif value >= 0.35 then
            qualityText = g_i18n:getText("el_ui_genetics_veryLow")
        else
            qualityText = g_i18n:getText("el_ui_genetics_extremelyLow")
        end
        ...
        table.insert(self.infos, { ["title"] = g_i18n:getText("el_ui_" .. keyText), ["value"] = qualityText, ["colour"] = { 1 - value / 1.75, value / 1.75, 0 } })
    ...
    elseif averageGenetics >= 1.1 then
        qualityText = g_i18n:getText("el_ui_genetics_good")
    elseif averageGenetics >= 0.9 then
        qualityText = g_i18n:getText("el_ui_genetics_average")
    elseif averageGenetics >= 0.7 then
        qualityText = g_i18n:getText("el_ui_genetics_bad")
    elseif averageGenetics >= 0.35 then
        qualityText = g_i18n:getText("el_ui_genetics_veryBad")
    else
...
function AnimalItemNew:getName()
    ...
    return animal.name or string.format("%s %s %s", EnhancedLivestock.AREA_CODES[animal.birthday.country].code, animal.farmId, animal.uniqueId)
