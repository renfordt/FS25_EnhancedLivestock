EnhancedLivestock_AnimalClusterHusbandry = {}
EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES = 50

local modDirectory = g_currentModDirectory

function EnhancedLivestock_AnimalClusterHusbandry:create(superFunc, xmlFilename, navigationNode, raycastDistance, collisionMask)
...
AnimalClusterHusbandry.create = Utils.overwrittenFunction(AnimalClusterHusbandry.create, EnhancedLivestock_AnimalClusterHusbandry.create)

function EnhancedLivestock_AnimalClusterHusbandry:deleteHusbandry(superFunc)
	if self.husbandryIds ~= nil then

		if self.animalIdToCluster == nil then
...
AnimalClusterHusbandry.deleteHusbandry = Utils.overwrittenFunction(AnimalClusterHusbandry.deleteHusbandry, EnhancedLivestock_AnimalClusterHusbandry.deleteHusbandry)

function EnhancedLivestock_AnimalClusterHusbandry:updateVisuals(superFunc, removeAll)
	...
	if EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES <= 0 or self.visualAnimalCount == EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES then
		return
	end

	local areaCode = EnhancedLivestock.getMapCountryCode()
	...
	if colours.earTagLeft == nil or colours.earTagLeft_text == nil or colours.earTagRight == nil or colours.earTagRight_text == nil then
		colours = animalSystem.baseColours
	end
	...
	for _, animal in pairs(animals) do
		...
		if self.visualAnimalCount >= EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES or i > #self.husbandryIds or animal.isDead or animal.numAnimals <= 0 or animal.uniqueId == "1-1" or animal.uniqueId == "0-0" or (animal.id ~= nil and animal.idFull ~= nil and animal.id ~= "0-0" and animal.visualAnimalIndex == nil) then
			continue
		end
		...
		if animalId == 0 then
			...
			while not useTempId and husbandryAnimalCount >= maxAnimalsPerHusbandry and i <= #self.husbandryIds and self.visualAnimalCount < EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES do
				i = i + 1
				if i > #self.husbandryIds then
					break
				end
				husbandryAnimalCount = self.husbandryIdsToVisualAnimalCount[self.husbandryIds[i]]
			end

			if i > #self.husbandryIds or (husbandryAnimalCount >= maxAnimalsPerHusbandry and not useTempId) then
				break
			end
			...
			while animalId == 0 and i <= #self.husbandryIds do
				...
				useTempId = false
				if i > #self.husbandryIds or self.husbandryIdsToVisualAnimalCount[self.husbandryIds[i]] >= maxAnimalsPerHusbandry then
					break
				end
		...
		if self.visualAnimalCount > EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES then
			break
		end
		if animalId == 0 then
			break
	...
	--print(string.format("EnhancedLivestock: %d visual animals loaded out of %d total animals for husbandry (%d max)", visualAnimalCount, #animals, EnhancedLivestock_AnimalClusterHusbandry.MAX_HUSBANDRIES))
...
AnimalClusterHusbandry.updateVisuals = Utils.overwrittenFunction(AnimalClusterHusbandry.updateVisuals, EnhancedLivestock_AnimalClusterHusbandry.updateVisuals)

function EnhancedLivestock_AnimalClusterHusbandry:getAnimalPosition(superFunc, id)
...
AnimalClusterHusbandry.getAnimalPosition = Utils.overwrittenFunction(AnimalClusterHusbandry.getAnimalPosition, EnhancedLivestock_AnimalClusterHusbandry.getAnimalPosition)

function EnhancedLivestock_AnimalClusterHusbandry:getClusterByAnimalId(superFunc, id, husbandryId)

	if husbandryId ~= nil then

		for index, husbandryIdFull in ipairs(self.husbandryIds) do
			if husbandryIdFull == husbandryId and self.animalIdToCluster[index] ~= nil and self.animalIdToCluster[index][id] ~= nil then
				return self.animalIdToCluster[index][id]
			end
		end
	...
	if string.contains(id, "-") then
		...
		if husbandryId ~= nil and animalId ~= nil and self.animalIdToCluster[husbandryId] ~= nil and self.animalIdToCluster[husbandryId][animalId] ~= nil then
			return self.animalIdToCluster[husbandryId][animalId]
		end
...
AnimalClusterHusbandry.getClusterByAnimalId = Utils.overwrittenFunction(AnimalClusterHusbandry.getClusterByAnimalId, EnhancedLivestock_AnimalClusterHusbandry.getClusterByAnimalId)